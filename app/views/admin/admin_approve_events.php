<?php
    ...
    include_once ROOT . '/app/controllers/approve_event.php';
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>public/assets/css/approve_event.css">
                ...
                <tr>
                    ...
                    <td class="d-flex justify-content-center">
                        <?php if ($row['status'] === 'pending'): ?>
                            <a href="?action=approve&id=<?= $row['event_id'] ?>" class="btn btn-approve btn-sm me-2" onclick="return confirm('Approve this event?')">Approve</a>
                            <a href="?action=reject&id=<?= $row['event_id'] ?>" class="btn btn-reject btn-sm" onclick="return confirm('Reject this event?')">Reject</a>
                        <?php else: ?>
                            <span>—</span>
                        <?php endif; ?>
                    </td>
                    <td><a href="<?php echo BASE_URL; ?>app/controllers/event_delete.php?id=<?= $row['event_id'] ?>" class="btn btn-delete btn-sm w-100" onclick="return confirm('Are you sure you want to delete this event?');">Delete</a></td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
